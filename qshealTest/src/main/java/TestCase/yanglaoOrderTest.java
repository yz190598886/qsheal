...
public class yanglaoOrderTest {
    ...
    private static final Logger log =Logger.getLogger(yanglaoOrderTest.class);
    ...
    public String name(){
        ...
        public void adduser() throws Exception {
            ...
            log.info("添加老人接口和参数："+addUserurl);
            log.info("接口返回："+ZFHttp.postMethod(addUserurl,ZFcookie));//添加老人
            ...
            log.info("充分接口和参数："+charge);
            log.info("接口返回："+ZFHttp.postMethod(charge,ZFcookie));//充分
            String first=ZFHttp.Jsonvalue(ZFHttp.getMethod(yue,ZFcookie),"data");
            System.out.println("余额："+first);
            Assert.assertEquals("1000.00",first);
            ...
            log.info("订单创建接口和参数："+Orderurl);
            log.info("接口返回："+QYHttp.postMethod(Orderurl,QYcookie));//创建订单
            ...
            log.info("派工接口和参数："+paidanurl);
            log.info("接口返回："+QYHttp.postMethod(paidanurl,QYcookie));//派工
            ...
            log.info("开始服务接口参数"+startServiceurl);
            log.info("接口返回："+AppHttp.postMethod(startServiceurl,Appcookie));//开始服务，上传坐标
            ...
            log.info("完成服务接口和参数："+completeServiceUrl);
            log.info("接口返回："+AppHttp.postMethod(completeServiceUrl,Appcookie));
            ...
            log.info("满意度接口参数"+updateServiceDegreeUrl);
            log.info("接口返回："+AppHttp.postMethod(updateServiceDegreeUrl,Appcookie));
